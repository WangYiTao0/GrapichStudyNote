...
- ShadowVolume
  ...
  - 原理
    ...
    1. 方法はいくつありますが。このDemoはDepthFailを使います。Carmack’s reverseと呼ばれます
    2. 通常レンダリングした後、ShadowVolumeをレンダリングする、Pass1はCull Front,内部をレンダリングする、StencilTest中で、深度テスト失敗した場合、このピクセルは
